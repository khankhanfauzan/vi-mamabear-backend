...
export class ProductUtils {
  ...
  async enrichOne(product: Product) {
    ...
    return {
      ...product,
      currentPrice: this.getDiscountedPrice(
        defaultPrice?.priceIdr!,
        defaultPrice?.discount as Discount,
      ),
  ...
  private getDiscountedPrice(priceIdr: Decimal, discount: Discount) {
    ...
    if (discount.isPercent)
      return priceIdr.mul(Decimal(100).minus(discount.amount)).div(100);
